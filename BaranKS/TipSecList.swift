...
//  BaranKS
//
//  Created by Baran Ambalaj A.Ş. on 5.10.2017.
...
class TipSecList: UIViewController, UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate, NSURLConnectionDelegate, XMLParserDelegate, NVActivityIndicatorViewable{
    
  //  var tableData = [Country]()
    
    @IBOutlet weak var topobjebaslik: UINavigationItem!
   // var topobjebasliktext: String!
  ...
  //  var puans = [String]()
    ...
    override func viewDidLoad() {
        ...
        topobjebaslik.title=MyClass.bolumadi1
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "myCell", for: indexPath) as! TipSecViewCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       ...
       //print(indexPath.row)
        ...
        if dialogs[indexPath.row]=="BRNBRNT119B35"
        {
          
        let tipsecviewk =  storyboard?.instantiateViewController(withIdentifier: "TipSecViewDetayID1") as! TipSecViewDetay1
       //tipsecviewk.ilkgelenpar = stexts[indexPath.row]
           MyClass.bolumadi2  = stexts[indexPath.row]
       self.present(tipsecviewk, animated: true, completion: nil)
        }
        else
        {
           
        let tipsecviewk =  storyboard?.instantiateViewController(withIdentifier: "TipSecViewDetayID2") as! TipSecViewDetay2
        //ipsecviewk.ilkgelenpar = stexts[indexPath.row]
            MyClass.bolumadi2  = stexts[indexPath.row]
        self.present(tipsecviewk, animated: true, completion: nil)
    ...
    func actions (veri:String) -> NSString
    {
        ...
        let celcius = veri
        var is_SoapMessage: String = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:cgs=\"http://tempuri.org/\"><soapenv:Header/><soapenv:Body><cgs:performanskriterlerilist><cgs:xparametre>\(celcius)</cgs:xparametre></cgs:performanskriterlerilist></soapenv:Body></soapenv:Envelope>"
        ...
        theRequest.addValue("http://tempuri.org/performanskriterlerilist", forHTTPHeaderField: "SOAPAction")
        ...
        let task = session.dataTask(with: theRequest, completionHandler: {data, response, error -> Void in
            ...
            Alamofire.request("http://onay.baranambalaj.com.tr/XMLFILE/"+parca1+"performanskriterlerilist.xml").response { response in
                ...
                do
                {
                    ...
                    if let items = xmlDoc.root["Table"].all{
                        ...
                        for item in items{
                            ...
                            self.dialogs.append(item["DIALOG"].string)
